#!/usr/bin/env python3
"""Phantom 6-DOF registration via fluorosim's Slang autodiff (single AP view).

Test of the registration loop:

  1. If pose.json from Isaac Sim is present, use its phantom_pos / carm_pos /
     phantom_quat as ground truth. Otherwise fall back to a synthetic-to-
     synthetic run with GT_TRANSLATION_MM env var.
  2. Render a target DRR at the ground-truth pose.
  3. Initialize the optimizer at a perturbed pose.
  4. Jointly optimize translation (3-DOF) and phantom rotation (3-DOF) by
     minimizing MSE between rendered and target DRR.  Gradients come from
     fluorosim's Slang shader autodiff.
  5. Convert recovered pose back to world frame and report error against GT.

6-DOF parameterization
-----------------------
  translation  (3,) mm  — world-frame displacement (carm_pos - phantom_pos)*1000
  phantom_rot  (3,) rad — phantom orientation (ZXY Euler, world frame)

Per optimizer step:
  R_phantom  = euler_zxy_to_matrix(phantom_rot)
  t_eff      = R_phantom.T @ translation
  euler_eff  = matrix_to_euler_zxy(R_phantom.T @ R_gantry)
  rendered   = drr_module(euler_eff, t_eff)

Note: single-view depth ambiguity means the Z translation (along the X-ray
beam) is poorly constrained (~0.4 mm error) while X/Y are accurate.  Use the
multi-view script (register_phantom_multiview.py) to collapse this.
...
def euler_zxy_to_matrix(e: torch.Tensor) -> torch.Tensor:
    ...
    R = Rz(rz) @ Rx(rx) @ Ry(ry)  — matches fluorosim's Slang shader.
    """
...
def matrix_to_euler_zxy(R: torch.Tensor) -> torch.Tensor:
    ...
    Uses atan2(sin_rx, cos_rx) to avoid arcsin's infinite gradient near ±90°.
    """
...
def main() -> int:
    ...
    print("=" * 60)
    print("register_phantom.py — 6-DOF single-view phantom registration")
    print("=" * 60)
    ...
    if isaac is not None:
        ...
        print("No pose.json — synthetic-to-synthetic mode.")
        print(f"  GT translation (mm):  {gt_translation_mm}")

    init_rot_rad = np.array(
        [math.radians(d) for d in INIT_ROT_DEG], dtype=np.float32
    )
    print(f"Initial offset (mm):     {INIT_OFFSET_MM}")
    print(f"Initial rot offset (deg): {INIT_ROT_DEG}")
    ...
    # AP view: gantry at (0, 0, 0)
    gantry_euler = torch.zeros(3, dtype=torch.float32)
    R_gantry     = euler_zxy_to_matrix(gantry_euler)  # = identity

    # --- Target DRR at GT pose -----------------------------------------------
    ...
    R_phantom_gt = euler_zxy_to_matrix(gt_rot_t)
    t_eff_gt = (R_phantom_gt.T @ gt_trans_t).tolist()
    r_eff_gt = matrix_to_euler_zxy(R_phantom_gt.T @ R_gantry).tolist()
    target_np = target_renderer.render(tuple(r_eff_gt), tuple(t_eff_gt))
    ...
    # --- Optimize ------------------------------------------------------------
    print("\n[4] Optimizing (6-DOF: translation + rotation)...")
    trace = []
    t_start = time.time()
    for it in range(N_ITERS):
        ...
        R_eff     = R_ph.T @ rot_gantry
        euler_eff = matrix_to_euler_zxy(R_eff)
        t_eff     = R_ph.T @ translation
        rendered  = drr_module(euler_eff, t_eff)
        loss      = ((rendered - target) ** 2).mean()
        loss.backward()

        # fluorosim Slang rotation backward may produce NaN; zero it out safely.
    ...
    with torch.no_grad():
        ...
        R_eff_final = R_ph_final.T @ rot_gantry
        euler_final = matrix_to_euler_zxy(R_eff_final)
        t_final     = R_ph_final.T @ translation
        recovered_np = drr_module(euler_final, t_final).cpu().numpy()
    ...
    if isaac is not None:
        ...
        with torch.no_grad():
            R_gt_f = euler_zxy_to_matrix(
                torch.tensor(gt_phantom_rot_euler, dtype=torch.float32)
            ).numpy().astype(np.float64)
        rot_err_geo = geodesic_angle_deg(R_gt_f, final_rot_mat)
    ...
    if isaac is not None:
        ...
        print(f"  World-frame err    (mm): {[round(v, 4) for v in ig['world_err_mm']]}")
        print(f"  World-frame ||err||(mm): {ig['world_err_norm_mm']:.4f}")
        print(f"  Rotation geodesic  (°):  {ig['rot_err_geodesic_deg']:.4f}")
    return 0
